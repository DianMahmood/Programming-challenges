...
descending_dictionary = {x: y for x, y in
                         sorted(process_results(file_contents).items(), key=lambda y: y[1][1] and y[1], reverse=True)}
...
if __name__ == "__main__":
    ...
    teams = list(descending_dictionary.keys())
    print(f"""{'Team ':^10} |{' Points ':^23}|{' GoalDifferences ':^20}|{' Wins ':^21}|{' Draws ':^20}|{' Losses ':^22}|
---------------------------------------------------------------------------------------------------------------------------""")
    for x in range(0, len(teams)):
        print(
            f"{premierleague_teams(descending_dictionary):<21}{totpoints(descending_dictionary):<23} {goal_differences(descending_dictionary):<20} {wins(descending_dictionary):<20} {draws(descending_dictionary):<20} {losses(descending_dictionary):>3}")
